...
export class Building {
    ...
    update(world: any, player: Player) {
        if (this.type === 'miner') {
            const now = Date.now();
        ...
        if (this.type === 'assembler' && this.recipe) {
            const hasInputs = this.recipe.inputs.every(input => {
                return this.inventory.filter(r => r.type === input.type).length >= input.amount;
            });
                ...
                if (this.craftingProgress >= this.recipe.craftTime) {
                    ...
                    this.recipe.inputs.forEach(input => {
                        for (let i = 0; i < input.amount; i++) {
                            const resourceIndex = this.inventory.findIndex(r => r.type === input.type);
                            this.inventory.splice(resourceIndex, 1);
                        }
                    });
                    // Produce output
                    const outputResource = new Resource(this.x, this.y, 50, 50, this.recipe.output.type);
                    const nextBuilding = world.getBuildingAt(this.x + this.width, this.y);
                    if (nextBuilding) {
                        nextBuilding.inventory.push(outputResource);
                    } else {
                        player.collectResource(outputResource.type, outputResource.width * outputResource.height / (world.tileSize * world.tileSize));
                    }
